...
} from "@/core/Components/Actions/hooks/useCollapsibility";
...
export interface ThActionsTokens {
  ...
  sheet?: {
    defaultSheet: Exclude<
      ThSheetTypes,
      ThSheetTypes.dockedStart | ThSheetTypes.dockedEnd
    >;
...
export interface ThSettingsSpacingPresets<
...
export type ActionKey<K extends CustomizableKeys> = K extends {
  action: infer A;
}
  ? A extends string
    ? ThActionsKeys | A
...
export type ThemeKey<K extends CustomizableKeys> = K extends { theme: infer T }
  ? T extends string
    ? ThThemeKeys | T
    : ThThemeKeys
  : ThThemeKeys;

export type SettingsKey<K extends CustomizableKeys> = K extends {
  settings: infer S;
}
  ? S extends string
    ? ThSettingsKeys | S
...
export type TextSettingsKey<K extends CustomizableKeys> = K extends {
  text: infer T;
}
  ? T extends string
...
export type SpacingSettingsKey<K extends CustomizableKeys> = K extends {
  spacing: infer S;
}
  ? S extends string
...
export type ThSettingsKeyTypes<K extends CustomizableKeys = DefaultKeys> = {
  ...
  [ThSettingsKeys.lineHeight]: ThSettingsRadioPref<
    Exclude<ThLineHeightOptions, ThLineHeightOptions.publisher>
  >;
...
export const createPreferences = <K extends CustomizableKeys = {}>(
  ...
  if (params.settings.spacing?.presets) {
    validateObjectKeys<ThSpacingPresetKeys, ThSpacingPreset<K>>(
      ...
      params.settings.spacing.presets.keys as Record<
        string,
        ThSpacingPreset<K>
      >,
    ...
    const adjustSpacingValue = (
      ...
      if (range) {
        ...
        if (adjustedValue < min) {
          console.warn(
            `Adjusting value ${value} for ${context.join(".")} to minimum allowed value ${min}`,
          );
          adjustedValue = min;
        } else if (adjustedValue > max) {
          console.warn(
            `Adjusting value ${value} for ${context.join(".")} to maximum allowed value ${max}`,
          );
          adjustedValue = max;
        }
      }
      ...
      if (step && range) {
        ...
        if (Math.abs(finalValue - adjustedValue) > Number.EPSILON) {
          console.warn(
            `Adjusting value ${value} for ${context.join(".")} to nearest step value ${finalValue}`,
          );
    ...
    for (const [themeName, spacingTheme] of Object.entries(spacingThemes)) {
      if (spacingTheme && typeof spacingTheme === "object") {
        ...
        if (hasAdjustedValues) {
          ...
          spacingThemes[themeName as keyof typeof spacingThemes] =
            adjustedTheme;
        }
      }
    }
...
export type ActionKeyType<K extends CustomizableKeys = DefaultKeys> =
  K["action"] extends string ? K["action"] : ThActionsKeys;
export type ThemeKeyType<K extends CustomizableKeys = DefaultKeys> =
  K["theme"] extends string ? K["theme"] : ThThemeKeys;
export type SettingsKeyType<K extends CustomizableKeys = DefaultKeys> =
  K["settings"] extends string ? K["settings"] : ThSettingsKeys;
export type TextSettingsKeyType<K extends CustomizableKeys = DefaultKeys> =
  K["text"] extends string ? K["text"] : ThTextSettingsKeys;
export type SpacingSettingsKeyType<K extends CustomizableKeys = DefaultKeys> =
  K["spacing"] extends string ? K["spacing"] : ThSpacingSettingsKeys;
